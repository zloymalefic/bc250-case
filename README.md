...
В репозитории есть браузерный viewer текущей компоновки корпуса. Он позволяет
вращать сборку, разносить детали, переключать заднюю крышку и независимо
скрывать элементы через легенду.

### Быстрый запуск

Требуются Python 3.10+ и современный браузер с WebGL 2. Для обычного просмотра
OpenSCAD не нужен: viewer использует готовые STL из `visualization/assets/`.

```sh
git clone <URL-РЕПОЗИТОРИЯ> bc250-case
cd bc250-case
python3 tools/dev_visualization.py
```

Открыть <http://localhost:8000/visualization/>. Сервер доступен только локально
на `127.0.0.1`. Для остановки нажать `Ctrl+C`. Другой порт:

```sh
python3 tools/dev_visualization.py --port 8080
```

Three.js загружается с jsDelivr, поэтому при первом открытии страницы требуется
доступ в интернет. Открывать `visualization/index.html` через `file://` нельзя:
браузер заблокирует загрузку STL.

### Управление viewer

- ЛКМ — вращение, колесо — масштаб, ПКМ — сдвиг камеры.
- Кнопки «Спереди», «Сбоку», «Сзади» и «3/4» выбирают готовый ракурс.
- «Разнести детали» показывает взаимное расположение компонентов.
- «Каркас корпуса» и «Внутренние компоненты» управляют крупными слоями.
- Каждая строка легенды отдельно скрывает или показывает соответствующий элемент.
- В блоке «Задняя крышка» выбирается горизонтальный, вертикальный вариант либо
  полностью снятая крышка.

### Обновление моделей

`cad/` и `cad-visualization/` являются исходниками, а
`visualization/assets/*.stl` — производными файлами только для рендеринга.
Обычный сервер не запускает OpenSCAD и не пересобирает CAD автоматически.

Полная сверка и обновление производных STL выполняются только явно:

```sh
tools/up-model
```

Команда собирает модели во временном каталоге, сравнивает геометрию и атомарно
заменяет только отличающиеся файлы в `visualization/assets/`. Исходные каталоги
она не изменяет. Проверка без замены файлов:

```sh
tools/up-model --check
```

Для пересборки требуется OpenSCAD в `PATH`. Установка на macOS:

```sh
brew install --cask openscad
```

На Ubuntu/Debian: `sudo apt install openscad`. Ручной совместимый entrypoint —
`tools/export_visualization_assets.sh`. После обновления STL запущенный viewer
сам перезагрузит страницу. Цвета и расположение объектов на сцене задаются в
`visualization/viewer.js` и не требуют изменения или повторного экспорта STL.

Полная инструкция, включая установку внешней mesh-модели BC-250 и решение
типичных проблем: [visualization/SETUP.md](visualization/SETUP.md).
